    ...
    public int hp
    {
        ...
        set
        {
            ...
            if(_hp <= 0)
            {
                _hp = 0;
                print("죽었다");
                //내가 죽었을 때 방장 소유권을 다른 클라이언트에게 넘긴다
                if(photonView.IsMine)
                {
                    //1. 죽어야하는 녀석이 마스터 클라이언트냐? (방에 대한 소유권을 가지고 있다)
                    //따라서 방을 나갈 때 반드시 마스터 클라이언트 권한을 이양해야한다.
                    //2. 로비씬 전환
                    //3. 방을 나가는 처리를 포토네트워크를 통해 해준다
